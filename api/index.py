...
def get_db_url():
    ...
    for key in ["DATABASE_URL", "POSTGRES_URL", "POSTGRES_PRISMA_URL", "POSTGRES_URL_NON_POOLING"]:
        ...
        if val:
            urls[key] = val[:30] + "..."
    result = os.environ.get("DATABASE_URL") or os.environ.get("POSTGRES_URL") or os.environ.get("POSTGRES_PRISMA_URL") or ""
    if result.startswith("postgres://"):
        result = result.replace("postgres://", "postgresql://", 1)
    return result, urls
...
if IMPORTS_OK:
    ...
    async def list_tags(user_id: int = Depends(get_current_user)):
        ...
        c.execute('''
            ...
            WHERE m.user_id = %s GROUP BY t.id ORDER BY count DESC
        ''', (user_id,))
        rows = c.fetchall()
        conn.close()
    ...
    async def create_message(msg: MessageCreate, user_id: int = Depends(get_current_user)):
        ...
        c.execute("INSERT INTO messages (user_id, content, created_at) VALUES (%s, %s, %s) RETURNING id",
                  (user_id, msg.content, created_at))
        msg_id = c.fetchone()[0]
        tag_names = detect_tags(msg.content)
        for name in tag_names:
            tag_id = ensure_tag(conn, name)
            c.execute("INSERT INTO message_tags (message_id, tag_id) VALUES (%s, %s) ON CONFLICT DO NOTHING",
                      (msg_id, tag_id))
    ...
    async def list_messages(page: int = 1, page_size: int = 10, tag: Optional[str] = None,
        ...
        if tag:
            c.execute('''SELECT DISTINCT m.id, m.user_id, u.username, m.content, m.created_at, m.is_processed
                FROM messages m JOIN users u ON m.user_id = u.id
                JOIN message_tags mt ON m.id = mt.message_id JOIN tags t ON mt.tag_id = t.id
                WHERE m.user_id = %s AND t.name = %s ORDER BY m.created_at DESC LIMIT %s OFFSET %s''',
                (user_id, tag, page_size, offset))
        else:
            c.execute('''SELECT m.id, m.user_id, u.username, m.content, m.created_at, m.is_processed
                FROM messages m JOIN users u ON m.user_id = u.id
                WHERE m.user_id = %s ORDER BY m.created_at DESC LIMIT %s OFFSET %s''',
    ...
    async def catch_all(path: str):
        return JSONResponse(status_code=500, content={"error": "Import failed", "detail": IMPORT_ERROR})

    @app.get("/api/debug")
    async def debug():
        return {"imports": "failed", "error": IMPORT_ERROR}
