...
async def create_message(msg: MessageCreate, user_id: int = Depends(get_current_user)):
    ...
    return {"id": msg_id, "user_id": user_id, "username": username,
            "content": msg.content, "created_at": created_at,
            "is_processed": False, "tags": tag_names}
...
async def list_messages(page: int = 1, page_size: int = 10, tag: Optional[str] = None,
    ...
    if tag:
        c.execute('''
            ...
            WHERE m.user_id = %s AND t.name = %s
        ''', (user_id, tag))
    else:
    ...
    if tag:
        c.execute('''
            SELECT DISTINCT m.id, m.user_id, u.username, m.content, m.created_at, m.is_processed
            FROM messages m
        ...
        c.execute('''
            SELECT m.id, m.user_id, u.username, m.content, m.created_at, m.is_processed
            FROM messages m
            JOIN users u ON m.user_id = u.id
            WHERE m.user_id = %s
            ORDER BY m.created_at DESC
    ...
    items = []
    for r in rows:
        item = {"id": r[0], "user_id": r[1], "username": r[2],
                "content": r[3], "created_at": r[4], "is_processed": r[5]}
